...
const TAG_COLORS: Record<string, string> = {
  ...
  Frontend: 'bg-pink-100 text-pink-600 dark:bg-pink-900/40 dark:text-pink-300',
  ...
  return (
    <Link href={`/til/${date}`}>
      <article className="group relative rounded-2xl border border-slate-100 bg-white p-5 shadow-sm transition-all duration-200 hover:-translate-y-1 hover:border-indigo-200 hover:shadow-md dark:border-indigo-900/30 dark:bg-slate-800/60 dark:hover:border-indigo-500/40 dark:hover:bg-slate-800">
        <time className="text-xs font-medium tracking-wide text-slate-400 dark:text-slate-500">
          {displayDate}
        </time>
        <h3 className="mt-1.5 line-clamp-2 text-sm font-semibold leading-relaxed tracking-wide text-slate-800 transition-colors group-hover:text-indigo-600 dark:text-slate-100 dark:group-hover:text-indigo-400">
          {title}
        </h3>
